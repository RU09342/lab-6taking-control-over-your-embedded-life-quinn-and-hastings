 ...
 * main.c
 */
int counter = 0; //8bit counter
int dir = 1; //direction of counter (up/down)
...
__interrupt void Timer0_A0_ISR(void) { //timer a0 interrupt service routine
    ...
    if(counter == 0x7F) //1111111
    {
        dir = -1; //countdown counter
    }
    if(counter == 0x00) //0000000
    {
        dir = 1; //up counter
    }
    ...
    counter+=dir;  //increment or decrement counter
}
